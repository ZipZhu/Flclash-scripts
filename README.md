...
*  **scripts14**：在scripts13基础上修改，经确认，flclash的覆写脚本与附加规则无法同时生效，因此添加了 `myManualRules` ，可以根据个人需要在 `myManualRules` 中添加规则。过滤掉原始配置中引用了本脚本未定义的 `rule-provider` 的 `RULE-SET` 规则，避免 "rule set [xxx] not found" 报错。
